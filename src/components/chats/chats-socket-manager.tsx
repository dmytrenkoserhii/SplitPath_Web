...
export const ChatsSocketManager = () => {
  ...
  React.useEffect(() => {
    const onNewPrivateMessage = (message: PrivateMessage) => {
      ...
      const chatPartnerId = message.from.id === userData.id ? message.to.id : message.from.id;
      ...
      queryClient.setQueryData<ChatPreview[]>([ReactQueryTags.CHAT_PREVIEWS], (oldData) => {
        const chatExists = oldData?.some((chatPreview) => chatPreview.userId === chatPartnerId);
                ...
                (oldData) => {
                  ...
                  const newPages = oldData.pages.map((page) => ({
                    ...
                    items: page.items.map((msg) => {
                      const status = readStatuses.find((rs) => rs.messageId === msg.id);
                      if (status && !msg.read) {
                        updated = true;
                        if (msg.from.id === userData.id && chatPartnerId === status.userId) {
            ...
            if (status.userId === userData.id) {
              const queryState = queryClient
                ...
                .find<InfiniteData<PaginatedResponse<PrivateMessage>>>({
                  queryKey: [ReactQueryTags.PRIVATE_CHAT_MESSAGES, chatPreview.userId],
                });

              const messageWasFromThisChatPartner = queryState?.state.data?.pages.some((page) =>
                page.items.some(
                  (msg: PrivateMessage) =>
    ...
    const onTypingStatus = ({
      ...
      queryClient.setQueryData<ChatPreview[]>([ReactQueryTags.CHAT_PREVIEWS], (oldData) => {
        ...
        return oldData.map((chat) => (chat.userId === userId ? { ...chat, isTyping } : chat));
      });
    };
