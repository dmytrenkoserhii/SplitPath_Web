...
export const chatsService = (): ChatsApi => {
  return {
    getChatPreviews: async (limit = 20): Promise<ChatPreview[]> => {
      const response = await xiorClient.get<ChatPreview[]>(`private-messages/chats?limit=${limit}`);
    ...
    sendMessage: async (payload: CreateMessagePayload): Promise<PrivateMessage> => {
      const response = await xiorClient.post<PrivateMessage>(`private-messages`, payload);
      return response.data;
    },
